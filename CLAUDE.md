...
이 저장소에서 Claude는 계획을 세우고, 승인된 구현 티켓을 superpowers 사이클로 구현하고, PR을 리뷰한다. 리서치와 계획 교차 리뷰는 Codex가 `AGENTS.md`에 따라 맡는다(ADR-0007, ADR-0013). 사이클의 절차·경로·승인 버전은 `docs/agents/project.md`의 "구현 사이클" 절이 정본이다.
...
- **리뷰 서브에이전트**: 티켓 등급이 요구하는 리뷰만(아래 "리뷰 게이트"). 판정은 원장에 남긴다.
- `CONTEXT.md`, `docs/adr/`, `docs/agents/`, 그리고 이 파일을 소유한다.

## 승인

- 필수 승인은 셋뿐이다: **머지**(`/code-review`까지 끝난 최종 HEAD), **main 직접 push**, **PR 밖의 지속 문서 편집**(스펙·ADR·`CONTEXT.md`·`CLAUDE.md`·`AGENTS.md`·`docs/agents/`). 이 셋은 변경 요지를 제시하고 명시적 승인을 받은 뒤 실행한다.
- 티켓 PR 안에 든 지속 문서 변경(Ruling이 스펙·ADR·`project.md`를 바꾸는 경우)은 머지 승인이 문서 편집 승인을 겸한다. PR 본문 "무엇을 남겼나"에 바뀐 문서와 요지를 적는다.
- 그 밖은 묻지 않는다. 구현 티켓은 OPEN 이슈의 `ready-for-agent` 라벨이 곧 승인이며, 범위는 그 티켓 안의 반복 개선, 계획 파일·원장·티켓 코멘트·PR 코멘트, 기능 브랜치 커밋·push, PR 생성까지. 구현 계획은 요약만 보고하고 바로 구현에 들어간다 — "이대로 시작할까요?"를 묻지 않는다. 범위 확장은 별도 티켓으로 만든다.
...
대상은 OPEN + `ready-for-agent`. `research` 라벨은 Codex 몫이다. `ui` 라벨은 Claude 사이클이며 구현자 브리프에 `.codex/skills/`의 UI 스킬 원문 경로를 넣는다(ADR-0013). 라벨이 없고 인수 조건이 있으면 구현 사이클. `needs-triage`·`needs-info`, 열린 차단 이슈, 담당자 있음, 스펙 "리서치 대기 #N" 값 참조는 착수하지 않는다. #13은 스펙이며 티켓이 아니다. 착수의 첫 쓰기는 클레임(`gh issue edit <n> --add-assignee @me`).
...
**티켓 등급과 리뷰 게이트(ADR-0012).** 계획 시점에 티켓을 `경량`·`표준`·`민감` 중 하나로 정해 계획 파일 머리에 적는다. 등급이 리뷰 횟수와 교차 리뷰 여부를 정한다. 표는 `docs/agents/project.md` "리뷰 게이트". 등급을 올리는 것은 자유이고 내리는 것은 원장에 Ruling으로 남긴다.

**계획 크기.** 계획 파일은 태스크마다 인터페이스·테스트 이름과 단언 요지·파일 목록·Ruling만 적고 구현 코드 본문은 넣지 않는다. 태스크는 4개 이하를 목표로 하고 계획 파일은 300줄을 넘기지 않는다. 넘으면 티켓 분할을 먼저 검토한다.
...
**리뷰 Minor.** 같은 PR에서 고치거나 버린다. 이슈로 만드는 것은 동작이 바뀌는 것뿐이며, 그때도 `needs-triage`로 하나만 만든다. PR 본문에 "후속" Minor 목록을 남기지 않는다.

**Ruling 범위.** 스펙 "개발 중 결정 항목"과 티켓 안 구현 세부(파일 배치, 내부 함수 분할, 테스트 구성). 개발 중 결정 값은 측정 절차·입력·선택 기준·결과 기록을 태스크로 쓰고 측정 결과로 정한다. 스펙·ADR·`project.md`의 의미를 바꾸는 Ruling은 같은 PR에 문서 변경 커밋으로 넣는다(머지 뒤 별도 반영 커밋을 만들지 않는다). ADR 충돌, "리서치 대기 #N" 값, 범위 확장, 도메인 용어 추가는 멈추고 티켓 코멘트로 묻는다.
...
인계는 `.scratch/handoff.md`로 하되, **사용자가 세션을 끝낸다고 말할 때만** 쓴다. 작업 단위가 끝났다는 것만으로는 인계하지 않는다 — 사용자가 같은 세션에서 다음 작업을 이어갈 수 있다.
...
- **지속 파일은 규칙만 적는다.** 규칙이 바뀐 경위·날짜·이전 규칙은 ADR과 git log에 두고 규칙 문장에 섞지 않는다. 매 세션 실리는 파일이라 줄 수가 곧 비용이다.

---
status: superseded by ADR-0013 (2026-09-27)
date: 2026-09-19
---
...
- `ui` 티켓의 구현은 Codex 서브에이전트가, 계획·검토·PR은 메인 Codex 세션이 맡는다(2026-09-23). 구현자와 검토자를 나눠 자기 구현을 자기가 검토하지 않게 하는 것이 이유이며, Claude 쪽 사이클(컨트롤러 ≠ 구현 서브에이전트)과 같은 구조다. #20 수정 라운드에서 Codex가 이미 이 방식으로 돌아간 것이 근거다. 절차는 `AGENTS.md` "구현 분업".
